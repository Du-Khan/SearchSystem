...
public:
    ...
    explicit SearchServer(const StringContainer& stop_words)
        ...
        for (const string& str : stop_words) {
            if (!IsValidWord(str)) {
                throw invalid_argument("содержит недопустимый символ");
            }
        }
    }
    ...
    void AddDocument(int document_id, const string& document, DocumentStatus status, const vector<int>& ratings) {
        ...
        if (documents_.count(document_id)) {
            throw invalid_argument("этот id уже есть");
        }
        ...
        doc_index.push_back(document_id);
    }
    ...
    int GetDocumentId(int index) const {
        if (!(index < GetDocumentCount() && index >= 0)) {
            throw out_of_range("id выходит за допустимый диапазон");
        }
...
private:
    ...
    QueryWord ParseQueryWord(string text) const {
        ...
        if (text.empty()) {
            throw invalid_argument("пустой текст");
        }
        ...
        if (text.empty() || text[0] == '-' || !IsValidWord(text)) {
            throw invalid_argument("ошибка в тексте");
        }
    ...
    Query ParseQuery(const string& text) const {
        ...
        if (text.find(" --"s)) {
            throw invalid_argument("двойной минус");
        }

        auto last = text[text.length() - 1];
        if (!(last - '-')) {
            throw invalid_argument("минус в конце строки");
        }
        for (const string& word : SplitIntoWords(text)) {
